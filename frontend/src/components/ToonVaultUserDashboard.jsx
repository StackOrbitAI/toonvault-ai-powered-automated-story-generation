...
export default function ToonVaultUserDashboard() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', background: COLORS.bg }}>
      <div style={{ color: COLORS.plum, fontWeight: 700, fontSize: 20 }}>Loading ToonVault...</div>
    </div>
  );

  const displayUser = userData || USER; // Fallback to mock for UI dev if needed
  ...
  return (
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: COLORS.bg, minHeight: "100vh", color: COLORS.ink }}>
      <Helmet>
        <title>User Dashboard | ToonVault - Your Creative Sanctuary</title>
        <meta name="description" content="Manage your stories, track your reading history, and generate amazing AI editorial content on your ToonVault dashboard." />
      ...
      <nav style={{
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px", height: 62, display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: COLORS.gold }}>{displayUser.coins || 0}</span>
          </div>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 14 }}>🔥</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: COLORS.rose }}>{displayUser.streak || 0}d</span>
          </div>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => { setProfileOpen(!profileOpen); setNotifOpen(false); }} style={{
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: COLORS.ink }}>{(displayUser.username || "User").split(" ")[0]}</span>
              <Badge plan={displayUser.plan || "Free"} size="small" />
            </button>
            {profileOpen && (
              <div style={{
                ...
                <div style={{ padding: "16px 18px", borderBottom: `1px solid ${COLORS.border}`, background: COLORS.plumLight }}>
                  <div style={{ fontSize: 22, marginBottom: 6 }}>{displayUser.avatar || "👤"}</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: COLORS.ink }}>{displayUser.username}</div>
                  <div style={{ fontSize: 12, color: COLORS.muted }}>@{displayUser.username}</div>
                  <div style={{ marginTop: 6 }}><Badge plan={displayUser.plan || "Free"} /></div>
                </div>
                ...
                <div 
                  ...
                  style={{ padding: "11px 18px", fontSize: 13, color: COLORS.danger, cursor: "pointer" }}
                >Log out</div>
              </div>
            )}
          </div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "24px 24px 48px", display: "flex", gap: 24 }}>
        ...
        <aside style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: "white", marginBottom: 2 }}>{displayUser.username}</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", marginBottom: 10 }}>@{displayUser.username}</div>
            <Badge plan={displayUser.plan || "Free"} />
            <div style={{ display: "flex", gap: 12, marginTop: 14, justifyContent: "center" }}>
              ...
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: "white" }}>{displayUser.streak || 0}</div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)" }}>Day Streak</div>
              </div>
              <div style={{ width: 1, background: "rgba(255,255,255,0.2)" }} />
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: "white" }}>{displayUser.storiesRead || 0}</div>
            ...
            <div>
              ...
              <div style={{
                ...
                <div>
                  ...
                  <h1 style={{ fontSize: 26, fontWeight: 800, color: "white", margin: "0 0 6px" }}>Hi, {(displayUser.username || "User").split(" ")[0]}!</h1>
                  <p style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", margin: 0 }}>You've read <strong style={{ color: "white" }}>{displayUser.dailyReadToday || 0}</strong> episodes today. Goal: <strong style={{ color: "white" }}>{USAGE.dailyReadingGoal}</strong></p>
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 14, marginBottom: 24 }}>
                ...
                <StatCard icon="✍️" label="Articles Generated" value={displayUser.articlesGenerated || 0} sub={`of ${USAGE.articlesLimit} this month`} color={COLORS.gold} />
                <StatCard icon="🔥" label="Read Streak" value={`${displayUser.streak || 0}d`} sub="Keep it going!" color={COLORS.rose} />
                <StatCard icon="🪙" label="Coins Balance" value={displayUser.coins || 0} sub="Earn more by reading" color={COLORS.gold} />
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginBottom: 24 }}>
                ...
                <div style={{ background: COLORS.card, borderRadius: 16, border: `1px solid ${COLORS.border}`, padding: "20px 22px" }}>
                  ...
                  <UsageBar label="Article Generation" used={displayUser.articlesGenerated || 0} total={USAGE.articlesLimit} icon="✍️" />
                  <UsageBar label="AI Panels" used={displayUser.aiPanelsUsed || 0} total={USAGE.aiPanelsLimit} icon="🎨" color={COLORS.rose} />
                </div>
              </div>
